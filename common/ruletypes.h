...
RULE_BOOL(Zone, TrilogyNewSpawnSpeedFromDB, true, "Trilogy: derive OP_NewSpawn (0x4921) walkspeed/runspeed from the NPC's DB values (matches the bulk OP_ZoneSpawns path). Off = legacy hardcoded 0.7/1.4 that broke fast movers (boats, mounts, roambox NPCs with custom speeds) when they were spawned mid-session and later invisibly stalled on the v29c client because position broadcasts exceeded the client's speed model. Flip off if this regresses any humanoid NPC rendering.")
RULE_BOOL(Zone, TrilogyBoatDiag, false, "Diagnostic: log every OP_NewSpawn (0x4921), bulk OP_ZoneSpawns (0x6121) inclusion, and outgoing A120 broadcast for NPC type 84250 (Firiona Vie Maiden's Voyage) — tracing the FV Maiden invisibility bug on Trilogy. Off by default; enable + #reloadrules to capture data during a repro. Zero cost when off.")
RULE_BOOL(Zone, TrilogyZoneSpawnsSplit, true, "Trilogy: split the bulk OP_ZoneSpawns (0x6121) zone-in emission into multiple packets of at most 100 NewSpawn_Struct entries each, mirroring EQClassic's SendZoneSpawnsBulk (Zone/Source/EntityList.cpp:913 `MAX_SPAWNS_PER_PACKET`). Fixes silent entity drops when a zone has >~500 NPCs: our prior single-mega-packet was deflated as one blob and fragmented across many wire datagrams, and v29c's decompress/parse would stop populating entities past a client-side entity cap — the mob that landed past the cap became server-alive but invisible (observed: Firiona Vie Maiden's Voyage at 727-NPC bulk_idx=523). Splitting per EQClassic keeps every entry within v29c's per-packet processing budget. Each chunk deflates + encrypts independently; the wire uses one OP_ZoneSpawns per chunk. Off = legacy single-packet path — flip off only if this regresses zone-in on lightly-populated zones.")
RULE_BOOL(Zone, TrilogyAirplaneLockTime, true, "Trilogy: when the player is in short_name 'airplane' (original Plane of Sky), pin the sky/lighting to a fixed EQ time-of-day so the zone stays in a permanent twilight/sunset mood. Only affects Trilogy clients; server-side zone_time keeps advancing so gameplay logic (spell durations, guard schedules) is unaffected.")
RULE_INT(Zone, TrilogyAirplaneLockHour, 17, "Trilogy: EQ hour (0-23, same convention as #time) to lock the airplane sky to when TrilogyAirplaneLockTime is on. 17 = red 5 PM sunset, 18 = purple 6 PM dusk, 19 = deep twilight.")
RULE_INT(Zone, TrilogyAirplaneLockMinute, 00, "Trilogy: EQ minute (0-59) to lock the airplane sky to when TrilogyAirplaneLockTime is on.")
RULE_CATEGORY_END()
...
RULE_BOOL(PlayerBots, CanClientsAggroPlayerBots, false, "Can clients attack / aggro PlayerBots ?")
RULE_BOOL(PlayerBots, PlayerBotsGiveXP, false, "Do PlayerBots give XP when killed by clients ?")
RULE_INT(PlayerBots, CorpseDecayTimer, 3600000,"How long do PlayerBot corpses stay in the world (3600000 = 1 hour).") 
RULE_INT(PlayerBots, InviteLevelGap, 5, "PlayerBots level above or below this value won't be able to be ^invite'd.")
...
RULE_INT(PlayerBotChat, PlayerReplyCooldownMs, 3000, "Mouth cooldown applied when a real PLAYER spoke, instead of PerListenerCooldownMs. Ambient bot-to-bot chatter shares the same per-listener budget, so without this a zone that has just talked among itself goes unresponsive to a human. Category cooldowns are also quartered for player-spoken lines. Output stays bounded by ResponseCapPerMessage.")
RULE_INT(PlayerBotChat, ConversationLockMs, 30000, "First-refusal window after answering a given speaker, in ms")
RULE_BOOL(PlayerBotChat, SpontaneousEnabled, true, "Allow bots to start their own conversations")
RULE_INT(PlayerBotChat, SpontaneousTickSec, 45, "Spontaneous scheduler tick, per zone, in seconds")
RULE_INT(PlayerBotChat, SpontaneousMaxPerZonePerHr, 20, "Conversation openers allowed per zone per hour")
RULE_INT(PlayerBotChat, SpontaneousMaxConcurrent, 3, "Concurrent bot conversations per zone")
RULE_BOOL(PlayerBotChat, LogDispatch, false, "Verbose per-dispatch logging (dev only; emission is high-volume)")
RULE_INT(PlayerBotChat, StaggerMinMs, 1000, "Minimum delay before a bot emits its response, in ms")
RULE_INT(PlayerBotChat, StaggerMaxMs, 4000, "Maximum delay before a bot emits its response, in ms")
...
RULE_INT(PlayerBotChat, DirectAddressStaggerMaxMs, 1200, "Stagger ceiling for a bot that was addressed by name, in ms. Answering to your own name after the same 1-4s as ambient chatter is the tell this removes. Clamped to StaggerMaxMs, so a value at or above StaggerMaxMs reproduces the old timing exactly.")
RULE_INT(PlayerBotChat, DirectAddressMaxResponders, 3, "How many name-addressed bots may answer one message, and the master switch for direct address: 0 turns the whole feature off (no rank bonus, no cooldown relief, no shortened reply) and restores proximity-only responder selection. Above 0 it is the griefer bound on the ResponseCapPerMessage exemption -- a line listing ten bot names must not become ten packets into a paced Trilogy queue. ResponseCapPerMessage 0 still silences everything regardless.")
RULE_INT(PlayerBotChat, RecencyPenaltyMs, 30000, "How long a listener that just spoke is handicapped in the responder ranking, in ms. Proximity outranks response weight by six orders of magnitude, so in a static camp the physically closest bot answered every message forever and its neighbours were mute scenery. 0 disables the penalty.")
RULE_INT(PlayerBotChat, LowManaPercent, 20, "Mana percentage at or below which a caster announces once, using the 'low_mana' category. The engine measures it, so the line is true by construction -- which is the only reason an 'oom' row is allowed to exist at all. Applies to anything with a real mana pool (GetMaxMana() > 0), so no caster list has to be maintained in content. Deliberately NOT suppressed by the combat gate: a caster running dry mid-fight is exactly when the group needs to hear it. 0 disables the proactive callout entirely and leaves the on-demand 'mana check' reply working.")
RULE_INT(PlayerBotChat, LowManaClearPercent, 45, "Mana percentage a caster must climb back to before it may announce low mana again. This is hysteresis, not a second threshold: without a gap above LowManaPercent the latch degenerates into a bare comparison and re-fires every sweep while the bar hovers on the line. Clamped to at least LowManaPercent + 1. The recovery itself is SILENT -- a zone of casters each announcing they are back up is the spam the latch exists to prevent.")
RULE_INT(PlayerBotChat, DuplicateUtteranceMs, 2000, "Window in which an identical line from the same speaker on the same channel counts as ONE utterance rather than several, in ms. This is not a nicety: v29c has no group-broadcast opcode, so the client sends group chat as one 0x0721 PER GROUP MEMBER, and a four-bot group turns one typed line into four messages milliseconds apart. Without this each copy opened its own conversation beat, so copy 2 stale-dropped the replies copy 1 had just queued while copy 1 had already reserved every candidate's mouth cooldown -- the group answered nothing at all. Also absorbs ARQ retransmits. 0 disables, which restores that behaviour exactly; raise it only if a client is seen fanning out more slowly.")
RULE_INT(PlayerBotChat, GroupVoiceMaxPerZonePerHr, 30, "Openers a GROUPED bot may start per zone per hour, counted separately from SpontaneousMaxPerZonePerHr. Group chat is scoped to at most six people and costs nothing zone-wide, so rationing it against ambient zone chatter is what made grouped bots read as silent: in a zone holding forty PlayerBots, four group bots drawing from one shared budget almost never won the roll. 0 disables the group opener pass entirely and leaves grouped bots to the ordinary zone budget.")
RULE_INT(PlayerBotChat, CombatReplyChance, 35, "Percent chance a bot answers ordinary chat while it is in combat, and the master switch for the whole combat gate: 100 restores the pre-19.5 behaviour completely -- no reduced odds, no lengthened stagger, and engaged bots may start spontaneous openers again. In combat means the bot itself IsEngaged() OR any member of its group is: a cleric that never takes aggro is still busy, and a group where only the tank goes quiet is worse than one that does not. Being addressed by name is exempt -- 19.1's whole point is that your own name cuts through, and that matters most mid-fight. Drops count as 'in-combat' in #pbchat stats.")
RULE_INT(PlayerBotChat, CombatStaggerPercent, 200, "Stagger multiplier applied to a reply queued while in combat, as a percent. A bot that is fighting takes longer to get to the keyboard. Applied before the StaggerMaxMs clamp, so it can never push a line past the Trilogy pacing budget, and before DirectAddressStaggerMaxMs, so a name-addressed reply stays fast mid-fight. 100 disables the multiplier while leaving the rest of the combat gate alone.")
RULE_INT(PlayerBotChat, CombatCalloutChance, 25, "Percent chance a Bot speaks its 'aggro' or 'victory' line on a combat event. This is the chorus guard, and it exists because the category cooldown cannot be one: cooldowns are PER LISTENER, so six bots engaging the same pull are six independent timers and all six call the incoming. The 'death' category deliberately ignores this and always fires -- a death line is one bot, once, and it is how the group learns somebody needs a rez. 100 means every callout is spoken; 0 silences aggro and victory without touching the DB rows.")
RULE_INT(PlayerBotChat, GroupVoiceBoostPercent, 250, "How much likelier a GROUPED bot is to start a spontaneous conversation than an ungrouped one, as a percent of the normal opener probability. The bots you actually adventure with should not be as quiet as zone scenery. A grouped opener also defaults to channel 2 instead of /say, so group small talk stays in the group -- rows carrying an explicit reply_channel (market, lfg, help) still broadcast, because that is a decision for the line. The per-zone hourly opener cap still bounds all of it. 100 disables the boost and the channel preference both.")
RULE_INT(PlayerBotChat, StaggerMsPerChar, 35, "Typing time charged per character of the line a bot is about to speak, in ms. Stagger ignored message length entirely, so a 90-character sentence and 'aye' both landed after the same 1-4s and the rhythm itself read as generated. The roll becomes random.Int(StaggerMinMs, min(StaggerMaxMs, StaggerMinMs + length * this)): StaggerMinMs is reaction time, this is the typing rate, and the spread widens with length the way a real one does. Never exceeds StaggerMaxMs, so the Trilogy pacing budget is unchanged. 0 restores the flat random.Int(StaggerMinMs, StaggerMaxMs) exactly. DirectAddressStaggerMaxMs still caps a name-addressed reply, so being named stays fast no matter how long the answer is.")
RULE_BOOL(PlayerBotChat, StaleEmissionDrop, true, "Drop a queued reply if the channel moved on while it sat in the stagger window. Without it a line answers a question nobody remembers asking, and StaggerMsPerChar makes that strictly more likely by lengthening the window. Scoped per listener and per channel, so it can only be tripped by a message that bot actually heard; replies belonging to the same conversation beat never invalidate each other, and two players telling the same bot stay separate. False restores the old behaviour, where every queued line always lands. Drops count as 'stale' in #pbchat stats.")
RULE_INT(PlayerBotChat, RecencyPenaltyDistance, 60, "Size of that handicap, expressed in DISTANCE UNITS: a bot that spoke a moment ago ranks as though it stood this much further from the speaker than it really does, decaying linearly to zero across RecencyPenaltyMs. Clamped to EarshotDistance. It is a soft preference -- a conversation-lock partner and a name-addressed bot both still win outright.")
RULE_CATEGORY_END()
