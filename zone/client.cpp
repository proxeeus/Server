...
//	ns->spawn.pvp		= GetPVP(false) ? 1 : 0;
	...
	ns->spawn.equipment.Slot[EQ::textures::weaponPrimary].Material = GetEquipmentMaterial(EQ::textures::weaponPrimary);
	ns->spawn.equipment.Slot[EQ::textures::weaponSecondary].Material = GetEquipmentMaterial(EQ::textures::weaponSecondary);
}
...
void Client::SendFactionMessage(int32 tmpvalue, int32 faction_id, int32 faction_before_hit, int32 totalvalue, uint8 temp, int32 this_faction_min, int32 this_faction_max)
{
	...
	if (tmpvalue == 0 || temp == 1 || temp == 2) {
		...
		MessageString(Chat::White, FACTION_BEST, name);
	} else if (faction_value <= this_faction_min) {
		MessageString(Chat::White, FACTION_WORST, name);
...
void Client::CheckTraditionalZonePoints()
{
	...
	if (m_trilogy_zonein_guard) {
		...
		if (gdx * gdx + gdy * gdy < kGuardDist * kGuardDist) {
			return;
		}
