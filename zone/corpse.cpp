	...
	m_loot_cooldown_timer(10)
{
	...
	for (auto &npcCorpseDecayTime: npcCorpseDecayTimes) {
		if (
			...
			if (npctype_id == RuleI(PlayerBots, PlayerBotId)) {
				...
				m_corpse_decay_timer.SetTimer(RuleI(PlayerBots, CorpseDecayTimer));
			}
			else {
	...
	if (npc->HasPrivateCorpse()) {
		...
		if (npctype_id == RuleI(PlayerBots, PlayerBotId)) {
			...
			m_corpse_decay_timer.SetTimer(m_corpse_decay_timer.GetRemainingTime() + 1000);
		}
	}
...
void Corpse::MakeLootRequestPackets(Client *c, const EQApplicationPacket *app)
{
	...
	if (IsPlayerBotCorpse()) {
		c->Message(13, "You cannot loot Player Bot corpses!");
