	...
	: NPC(npcTypeData, nullptr, glm::vec4(), Ground, false), rest_timer(1), ping_timer(1)
{
	...
	if (GetClass() == Class::Monk || GetClass() == Class::Necromancer || GetClass() == Class::ShadowKnight)
		forget_timer.Start();
...
void Bot::BotRangedAttack(Mob* other) {
	...
	if (RangeWeapon->ItemType == 7) // Throwing
	{
		if (!IsAttackAllowed(other) || IsCasting() || DivineAura() || IsStunned() || IsMezzed() || (GetAppearance() == eaDead))
			return;
		SendItemAnimation(other, RangeWeapon, EQ::skills::SkillThrowing);
		...
		else
		{
			...
			LogCombat("Shooting %s with bow %s (%d) and arrow %s (%d)", other->GetCleanName(), RangeWeapon->Name, RangeWeapon->ID, Ammo->Name, Ammo->ID);
			if (!IsAttackAllowed(other) || IsCasting() || DivineAura() || IsStunned() || IsMezzed() || (GetAppearance() == eaDead))
				return;
...
void Bot::DoClassAttacks(Mob *target, bool IsRiposte) {
	...
	if (ma_time) {
		if (GetLevel() >= 10 && GetLevel() <= 19) // Use this to keep Tiger Claw on its own separate level-range and not as another attack
		{
			switch (GetClass()) {
			case Class::Monk: {
...
void Bot::GenerateSpecialAttacks() {
	...
	{
		if( (GetClass() == Class::Monk || GetLevel() >= RuleI(Combat, MonkOldTripleAttackLevel))
			||
