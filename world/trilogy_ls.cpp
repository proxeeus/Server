                                   ...
                                   const uint8_t* payload, uint32_t plen)
{
	...
	case LS_OP_LoginInfo: {
		// Client sends 40-byte DES block — accept unconditionally.
		// Real DES auth and account lookup happen at port 9000 in
		// HandleSendLoginInfoPacket via Trilogy::DecryptLoginBlock.
		// SessionId "LS#1" satisfies the client without a real account ID.
		uint8_t     buf[21]{};
		const char* sid = "LS#1";
		memcpy(buf,      sid,     strlen(sid)); // session_id[10]
		memcpy(buf + 10, "unused", 6);          // unused[7]
		// buf[17..20] = little-endian uint32 = 4
	...
	case LS_OP_SessionKey: {
		// Client selected a server and sends its address back.
		// We respond with a 17-byte key: [0x00][15 printable chars][0x00].
		// The key is not validated at port 9000 (DES auth is used instead).
		uint8_t key[17]{};
		for (int i = 1; i <= 15; ++i) {
			int y = (rand() % 62) + 48;
			if (y > 57) y += 7;
			if (y > 90) y += 6;
			key[i] = static_cast<uint8_t>(y);
		}
...
{
	const WorldConfig* cfg  = WorldConfig::get();
	std::string        name = cfg->LongName;
	std::string        addr = cfg->WorldAddress;
	if (name.empty()) name = "EQEmu";
